      ...
      <div className="mb-6">
        ...
        <div className="space-y-2">
          {PRESET_SIZES.filter(s => s.id !== 'custom').map(s => (
            <label key={s.id} className={radioRow(selectedPreset === s.id)} onClick={() => setSelectedPreset(s.id)}>
              <div className="flex items-center gap-3">
                <div className={radioCircle(selectedPreset === s.id)} />
                <div>
                  <p className={`text-sm font-medium leading-tight ${selectedPreset === s.id ? 'text-gray-900' : 'text-gray-700'}`}>
                    {s.label}
                  </p>
          ...
          <label className={radioRow(selectedPreset === 'custom')} onClick={() => setSelectedPreset('custom')}>
            <div className="flex items-center gap-3">
              <div className={radioCircle(selectedPreset === 'custom')} />
              <span className={`text-sm font-medium ${selectedPreset === 'custom' ? 'text-gray-900' : 'text-gray-700'}`}>
                Custom size
              </span>
            </div>
          </label>
          <AnimatePresence>
            {isCustomSize && (
              ...
              >
                <div className="grid grid-cols-2 gap-3 pt-2 pl-7">
                  <div>
